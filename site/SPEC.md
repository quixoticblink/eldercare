...
Hyperlinked card with a serif name and a one-line description in soft text. Hover lifts slightly. The `.dormant` modifier dims cards for folders that aren't active yet. As of 2026-07-11 no card carries it — strategy, prototype and pitch have all been activated. The modifier stays in the CSS for the next folder that gets added.
...
### `.signal` (field-interview signals — and, since 2026-09-04, feedback signals)
...
### `.artefact-card` + `.proto-modal` (the build showcase)

Used in "The Build" section (`#showcase`). An `.artefact-grid` holds two `.artefact-card`s — flat white cards with an accent top-border, a mono `.kind` label, a serif `.name`, one paragraph, and a `.btn-row` of `.site-btn` buttons (solid accent primary, `.ghost` secondary). The deck card carries a `.cover` image (a render of the title slide, stored in `site/assets/`).

The prototype button opens `.proto-modal` — the site's one piece of real interactivity: a full-viewport overlay (`--navy-deep` at 85%) containing a navy `dialog-bar` (mono label, full-screen link, close button) and an `<iframe>` running `assets/kakis-prototype.html`. The iframe `src` is set lazily on first open. Close via button, backdrop click, or Escape; focus is moved to the close button on open and restored on close; body scroll locks while open. Keep this the *only* modal on the site — if a second one is ever needed, generalise the pattern first in this SPEC.
...
2. **Journey.** The narrative arc. Currently a timeline from May 19 onward to Demo Day, with past entries in accent and future entries muted. Runs through Aug 3 (NCSS/Vanguard tabletop guidance), Aug 9 (the app going live), Aug 18 (NCSS desk review) and Aug 21 (the Table Top Exercise), then a future Round 2 entry and Demo Day.
...
8. **What the Seniors Said (`#feedback`).** The August feedback round — NCSS desk review (Aug 18) and the Table Top Exercise with eight seniors (Aug 21) — as seven `.signal` blocks, one verbatim quote, a `.callout` naming what was *not* tested, and links to the two wiki registers. Sits after The Build because it is feedback on the build; reuses the `.signal` component from What We Heard rather than introducing a new one.
9. **How We Work / Knowledge.** The wiki structure as cards, plus the four-part rubric mapping.
10. **Footer.** Project description, navigation links, colophon.
...
- **2026-09-20 (pitch deck, presented version)** — the deck card now serves the September deck as a single PDF (`assets/Kakis-DemoDay-Pitch.pdf`, 16 pages): the presented slides only, the eleven hidden working slides removed before export. The `.pptx` download button and file are gone from the site; the full source deck with its hidden slides lives in `knowledge/pitch/`. New `assets/deck-cover.jpg` from slide 4 (the "Kakis. Trusted respite-activation platform" title). Card label → "Demo Day, September 2026". Triggered by user: use the attached deck, visible slides only, PDF only.
- **2026-09-20 (explainer video)** — `#showcase` `.artefact-grid` gains a fourth `.artefact-card`: "How Kakis works · 1-minute video", a native `<video class="cover" controls preload="none" playsinline>` with a poster (`assets/kakis-how-it-works-poster.jpg`) and the file itself (`assets/kakis-how-it-works.mp4`, 7.7 MB, 1080p, 60 s, captioned) served from the repo; one ghost button to download. One CSS line gives `video.cover` a 16:9 box. `preload="none"` so the page does not fetch 7.7 MB until someone presses play. If the file ever grows past ~20 MB, move it to YouTube and swap the `<video>` for an `<iframe>`; nothing else on the card changes.
- **2026-09-20 (feedback photo)** — `#feedback` gains one `img.photo` of the venue after the lede (`assets/care-corner-261a.jpg`, the Care Corner AAC entrance at Toa Payoh 261A, no faces), matching `#beta`. The wiki already holds the same door as `images/2026-08-21-tte-04-care-corner-toa-payoh-261a.jpg`.
- **2026-09-20 (sync pass)** — `#showcase` lede now says "two rounds of senior feedback built in: the August list on the 5th, the befrienders' list on the 20th"; the 5 Sept journey entry's "still deliberately not built" line put in the past tense. Nothing else was stale against `knowledge/prototype/roadmap.md`.
- **2026-09-20 (v1.8 shipped)** — **Journey timeline:** new past entry "September 20, 2026 · Build — The befrienders' list, small half" (five arc chips, `.final` = "v1.8 live") before the future Round 2 entry, which now says v1.8. **`#showcase`:** `.sub-h` → "What changed in v1.6, v1.7 and v1.8, and what is next"; Shipped card label "5 and 20 September 2026", new v1.8 lead sentence and a link to `journal/2026-09-20-v1.8-build.md`; Roadmap card re-ordered to lead with the three Lions Befrienders items (many seniors per caregiver, what each side sees, danger alert) and "Further out" absorbs repeat bookings, reimbursement, Singpass/PDPA, Malay. **`#beta`** callout rewritten in the past tense ("What changed next"). Colophon → "2026-09-20 (evening)". No new components. Wiki side first: `journal/2026-09-20-v1.8-build.md`, `prototype/plans/v1.8-befrienders-round.md`, `roadmap.md`, buckets, `app/SPEC.md` v1.8. Triggered by the v1.8 deploy.
- **2026-09-20 (Lions Befrienders beta)** — new **`#beta`** section after `#feedback`, same anatomy (`.eyebrow`, h2, `.lede`, intro paragraph, six `.signal` blocks with one `.signal-quote`, a `.callout`, a sources paragraph) plus one `img.cover` of the venue (`assets/lions-befrienders-170.jpg`, no faces). Nav gains "Beta"; footer index gains "The befrienders' round". **Journey timeline:** new past entry "September 11, 2026 · Beta — Lions Befrienders try v1.7" (five arc chips, `.final` = "a v1.8 list, and two decisions") between the Language entry and the future Round 2 entry. Colophon → 2026-09-20. No new components; `.signals` reused for the second time by design (same content type as `#feedback`). Wiki side first: `journal/2026-09-11-lions-befrienders-beta.md`, `prototype/lions-befrienders-2026-09-11-feedback.md`. Triggered by user: record the beta session on the site.
- **2026-09-05 (v1.7 language)** — **Journey timeline:** new past entry "September 5, 2026 · Language — The same app, in Mandarin" (one paragraph, five arc chips, `.final` = "v1.7 live") after the v1.6 Build entry; the v1.6 entry's "still deliberately not built" list drops Mandarin; the future Round 2 entry now says v1.7 and "the door and booking changes and the Mandarin screens together". **`#showcase`:** `.sub-h` → "What changed in v1.6 and v1.7, and what is next"; Shipped card gains a 中文 (v1.7) lead sentence and a link to `journal/2026-09-05-v1.7-language.md`; Roadmap card's "Mandarin, then Malay" → "Malay" (same dictionary, waits for a Malay-speaking reviewer). Colophon → "2026-09-05 (evening)". No new components. Wiki side first: `journal/2026-09-05-v1.7-language.md`, `prototype/roadmap.md` (v1.7 entry and the round-2 note), buckets file ("v1.7 — shipped" line), `prototype/kakis-app.md`, `maps/timeline.md`, `knowledge/README.md`, `app/SPEC.md` v1.7. Triggered by the v1.7 deploy.
- **2026-09-05 (tone + roadmap)** — **`#feedback`** h2 changed *"The concept held. The onboarding didn't."* → *"The concept landed. The seniors showed us what to build next."*; intro paragraph now says eighteen of the recommendations shipped as v1.6; Aug 21 journey entry body reframed from "what did not hold" to "the most useful gift a first test can give", `.final` chip *"a build list, not a verdict"*. **`#showcase`** gains a second `.sub-h` ("What changed in v1.6, and what is next") over a two-card `.strategy-grid` reused from the Strategy section — *Shipped · 5 September 2026* and *Roadmap · deliberately not yet* — linking to the SPEC change log, the build journal, and the new `knowledge/prototype/roadmap.md`. Section 5's `.sub-h` note ("use sparingly") stands; this is the second use in the same section, justified because the walkthrough and the changelog are different kinds of content. Triggered by user: phrase the feedback headline positively; show what was updated and what is on the roadmap.
- **2026-09-05 (v1.6 shipped)** — **Journey timeline:** new past entry "September 5, 2026 · Build — The August feedback, built in" (two paragraphs, five arc chips, `.final` = "v1.6 live") between the Aug 21 entry and the future Round 2 entry; Round 2 body shortened to point at v1.6. **`#showcase`:** lede gains "and, since 5 September, with the August feedback built in"; the kaki paragraph of the three-journey walkthrough rewritten for hours-per-day availability, photo and certificates, on-the-way, and the two-way door check (kaki code, then start code). Footer colophon → 2026-09-05. No new components. Wiki side first: `journal/2026-09-05-v1.6-build.md`, `prototype/kakis-app.md` (two new "what went wrong" entries), buckets file status line, README status tick, `app/SPEC.md` v1.6 change log. Triggered by the v1.6 deploy.
- **2026-09-04 (August feedback round)** — **New top-level section `#feedback` ("What the Seniors Said")** between The Build and Knowledge: eyebrow/h2/lede, one honesty paragraph on sample size, seven `.signal` blocks (the wall before the product; language as a gate; money questions are about rules; kaki sees too much, caregiver too little; refresh is not a notification; booking wants exact times; the operator reviews for operating cost), one verbatim participant quote, a `.callout` naming that the crisis triggers were not tested, and links to the two wiki registers. No new component — `.signal` reused (section 5 heading amended). Header nav and footer gained a "Feedback" link. **Journey timeline:** two new past entries, Aug 18 · NCSS review ("An operator read the app screen by screen", four arc chips, `.final` = "start code, both directions") and Aug 21 · Table Top Exercise ("Eight seniors, their own phones, the live app", five chips, `.final` = "the wall is before the product"); the Aug 20 `future` milestone entry removed (met in substance on Aug 21, said so in that entry) and replaced by a `future` "September 2026 · Round 2" entry. **Hero meta** phase: "pilot design &amp; prototype" → "tested with seniors · round 2 next". **Knowledge index:** Prototype card description refreshed; two new kb-cards (Table Top Exercise register, NCSS desk review), GitHub URL scheme. Footer colophon → 2026-09-04. Section 7 flow renumbered. No photos on the site: the four session photos in `knowledge/images/` show identifiable participants and stay in the wiki until consent for public use is confirmed. Wiki side done first in the same pass: `journal/2026-08-18-ncss-app-review.md`, `journal/2026-08-21-tabletop-vanguard-ncss.md`, `prototype/ncss-app-review-2026-08-18.md`, `prototype/tabletop-2026-08-21-feedback.md`, plus README/timeline/purpose/kakis-app/care-corner/humans/images updates. Triggered by the two feedback documents and the session photos.

- **2026-08-09 (consistency sweep)** — No design change; five stale strings corrected after a cross-file audit found the docs disagreeing with each other. **`#journey`** intro and **`#hmw`** h2: *"The question has moved twice"* → *"four times"* — the `.hmw-history` chronology below it had already carried five steps since 2026-07-11, so the headings had been contradicting the component they introduce. `#hmw` lede dropped "the two field encounters", which was the same undercount. Closing paragraph: *"The four dated iterations"* → *"five"*. Footer colophon `2026-07-11` → `2026-08-09`. `site/README.md` and this SPEC's own `.kb-card` component note corrected — both still said prototype and pitch remained dormant, four weeks after they were activated. Same pass fixed the wiki side (`knowledge/README.md`, `reframing/README.md`, `purpose.md`, all of which still named the superseded Jul 8 crisis-only HMW as current) and the app side (`app/SPEC.md` sections 1/3/4/5/6/9.4 had drifted behind v1.2–v1.5; `smoke.py` now counts its own assertions after that figure reached six different values across five documents). Triggered by a root-README update that surfaced the contradictions.
...
- **2026-07-11 (prototype + pitch wake)** — The prototype and pitch folders activated six weeks ahead of the August milestone, and the site now reflects it. **Journey timeline** gained a "July 11, 2026 · Build" past entry between the Broadening entry and the Jul 20 Masterclass — the first interactive prototype (working name *Kakis*: three personas, eighteen screens, agent-run ops console, planned-or-urgent activation per the Jul 11 HMW) and the first sixteen-slide Demo Day deck. Five arc chips, `.step.final` = "prototype + pitch wake". **Aug 20 future entry** reworded — the prototype window now *hardens* the Jul 11 build rather than opening cold, and "crisis trigger" became "a need arising" to match the broadened HMW. **Knowledge index:** Prototype and Pitch kb-cards un-dormanted (Recipe 5) with real descriptions; links unchanged (GitHub README scheme). **Hero meta** phase updated "strategy & pilot design" → "pilot design & prototype". Wiki side already done in the same pass: `knowledge/prototype/` (kakis-prototype.html + kakis-design-brief.md + README) and `knowledge/pitch/` (Kakis-DemoDay-Pitch.pptx + README). Triggered by the prototype and pitch-deck build landing in the knowledge base.
- **2026-07-11 (attribution pass — Vanguard = crisis, team = need arises)** — Follow-up to the broadening: sharpened who-owns-what across every place the two framings appear. Vanguard (via Angeline) was the origin of the *"crisis hits"* framing — she named crisis as the sharpest unmet pain and listed six triggers. The sprint team is what reframed to *"need arises"* on Jul 11. **Jul 8 timeline entry** ("A new HMW, narrowed") — headline changed to *"...narrowed to Vanguard's framing"*, body rewritten as *"Not a fresh reframe — an adoption. We took Vanguard's own framing..."* **Jul 11 timeline entry** — headline changed from *"Crisis is one end of the spectrum..."* to *"Vanguard said crisis; the team reframed to need"*; body rewritten with explicit attribution; arc chips gained *"Vanguard's framing = crisis"* and *"team catches it as too tight"*. **hmw-history Jul 8 step** label changed to *"Vanguard's framing narrows the WHO,"* body rewritten. **hmw-history Jul 11 step** label changed to *"Team reframes from crisis to need — current,"* body adds *"Vanguard said crisis; the team reframed to need."* **"How the question moved" prose** rewritten with the same attribution — *"we adopted Vanguard's own framing... as a team we caught our adoption of Vanguard's framing as too tight... Vanguard said crisis; the team reframed to need arises."* **Strategy body paragraph** — *"Crisis triggers gave us the initial wedge"* → *"Vanguard's crisis triggers gave the pilot its initial wedge... the team's Jul 11 reframe added planned respite alongside them from day one."* Triggered by user note: "the vanguard said crisis hits example .. we as a group have reframed to need arises."
- **2026-07-11 (HMW broadening + Vanguard-location correction + privileged-content removal)** — Three-day-old crisis-only framing corrected. **Hero H1** and lede rewritten around *"when a need arises"* (planned respite for meetings, vacations, appointments AND urgent respite for the six Vanguard-named crisis triggers). **Meta description**, `og:title`, `og:description` updated to match. **HMW statement** DO changed *"when a crisis hits"* → *"when a need arises."* **HMW grid** WHO tightened to *"Caregivers of the relatively healthy elderly — especially in need or in crisis"* (user's phrasing); DO cell rewritten with concrete examples (a meeting, a vacation, an appointment, a crisis). **`.hmw-history`** gained a new Jul 11 step with the `.current` modifier; Jul 8 step keeps its label but loses `.current`. **"How the question moved"** prose extended with a fourth beat covering the Jul 11 correction. **Callout** rewritten to name the honest tradeoff — two demand curves (planned + urgent) in one product. **Strategy section H2** changed from *"crisis-activated marketplace"* → *"need-activated marketplace"*; lede + body paragraph updated to hold both crisis and planned respite on the same supply layer; Vanguard callout inside the strategy section retained the six crisis triggers Vanguard actually named and added the Jul 11 broadening note. **Journey timeline** gained a new "July 11, 2026 · Broadening" past entry between the Jul 8 afternoon entry and the Jul 20 future entry (5 arc chips, `.step.final` = "HMW broadened"). **Knowledge index** gained a new kb-card for `hmw-2026-07-11-broader-need.md`; the "HMW (current)" card description updated to reflect the July 11 framing. **Footer** description updated. **Vanguard-location correction:** the July 8 site visit was at Vanguard's Hougang care home, *not* Pasir Ris (Pasir Ris is the estate they're planning for the eldercare prototype pilot). The Jul 8 timeline heading changed *"Two hours in Pasir Ris"* → *"Two hours at Vanguard Care Home, Hougang"*; the entry body clarifies both locations. The "How the question moved" prose gained the same parenthetical clarification. Every remaining Pasir Ris mention refers to the pilot estate and is correct. **Privileged-content removal:** the May 19 timeline entry's list of eleven researched angles dropped *"the NCSS GrabCare postmortem"* — that reference should not appear on the public site. **Wiki sync (per Prompt 6 discipline):** `knowledge/reframing/hmw-current.md`'s status note repointed to the new iteration; `knowledge/reframing/hmw-2026-07-11-broader-need.md` created (full six-section entry: what-changed, why-the-correction, three slots, does-well, doesn't-do-well, devil's-advocate seeds, open questions, connects-to). Triggered by user feedback: "i dont only want to focus on when a crisis hits — it can be when needed."
...
- Forms or interactivity beyond anchor links — with one deliberate exception as of 2026-07-11: the prototype modal in `#showcase` (see section 5). The site is otherwise a read.
