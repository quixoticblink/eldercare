# Kakis app — SPEC v1.8 (pilot build)
...
│       ├── i18n.js        ← EN / 中文 dictionary + display names for data values (v1.7)
│       ├── ui.js          ← DOM helpers, components (chips, cards, toasts), UI.t/UI.v ONLY
...
Errors: `{detail}` with proper status codes; frontend surfaces via toast. **v1.7:** errors a senior can run into at sign-in or at the door (`code_wrong`, `code_expired`, `too_many_tries`, `not_approved`, `session_expired`, `window_passed`, `date_passed`, `date_too_far`, `kaki_code_wrong`, `start_code_wrong`) are raised as `errors.KakisError` and come back as `{detail, error}` — `detail` stays the English sentence (logs, smoke, coordinator), `error` is the stable code `UI.terr()` maps to the person's language.
...
**Language (v1.7).** Caregiver and kaki screens exist in English and Simplified Chinese; the coordinator console is English only, by design (§9.3). Rules:

- Every sentence a caregiver or kaki reads lives in `js/i18n.js` under a short stable id and is rendered with `UI.t(id, vars)`; a missing `zh` entry falls back to English, never to a blank. `views/auth.js`, `caregiver.js`, `kaki.js` and `help.js` contain no literal English. `views/admin.js` never calls `UI.t` and never reads `I18N`.
- **Data stays English.** Service, trigger, language, status, tier, weekday and half-day values are stored and sent exactly as before; only the label changes, through `UI.v(kind, value)` (and `UI.chipGroup/chipMulti(…, kind)` keep the English value in `data-v`). A translated label never reaches the API.
- **Nothing a person typed is translated:** names, addresses, notes, reasons, reports, care-plan fields, certificate names.
- The choice lives in `localStorage.kakis_lang` (so the sign-in screen can be Chinese before anyone signs in) and in `users.lang` (so it follows the person to another phone and decides the language of their messages). On boot the server's record wins, then the phone's stored choice, then the phone's own language (`navigator.language` starting `zh`); only an explicit choice is written back. An admin session forces `en` without touching the phone's stored choice.
- The toggle (`#langBtn`, brand bar) is present on every caregiver and kaki screen including sign-in and the waiting screen, and hidden for admins. A tap re-renders the current screen; typed input on the sign-in screens is kept.
- Glossary: *Kaki* stays Kaki · start code 开始码 · kaki code Kaki 验证码 · caregiver 照顾者 · visit 探访 · coordinator 协调员 · care plan 照护计划. Register: Simplified Chinese as spoken in Singapore, short plain sentences, no idioms.
- Backend: notifications pick the template by the recipient's `users.lang` (`services/notify.py`; the emergency-contact SMS follows the caregiver's language); the English subject is what reaches `audit_log`. The help bot answers in the language of the question (`llm.is_zh` → `HELP_GUIDE_ZH`; the provider prompt says so). Service names stay English inside Chinese messages: they are the words on the coordinator's console and on the booking.
...
- After a first sign-in you land on a **waiting screen** until the coordinator
  ...
  approval — **optional** (v1.8.1): the button says *Add a certificate*, the
  hint says so, and the profile has *Skip for now* back to the waiting screen.
  Certificates help the coordinator match sooner; they are never a gate.
...
- **中文 / English (v1.7).** The button at the top right switches every
  ...
  between English and Simplified Chinese. The app remembers the choice on this
  phone and on your account, so it is the same the next time and on another
  phone. A phone set to Chinese starts in Chinese. Names, notes and anything
  you type stay exactly as typed.
...
The console is **English only, by design** (v1.7): there is no language button
for admins and the screens stay English even on a phone set to Chinese. The
coordinator reads one language across the console, the audit log and every
message subject, whatever language the person on the other end chose.
...
- **v1.8.1 (2026-09-20) — certificates read as optional.** Nothing in the backend ever required a certificate, but the waiting screen said "the coordinator checks these before approving you" and the profile had no way back. Now: *Add a certificate* (quiet button) with "Optional…" under it, profile subtitle "certificates are optional", empty state "None yet, and that is fine", and a *Skip for now* button on a pending kaki's profile. en + zh. e2e updated; assets bumped to `?v=1.8.1` so cached phones fetch the new copy.
- **v1.8 (2026-09-20) — the befrienders' round.** Ten small items from the Lions Befrienders session of 11 Sept (`knowledge/prototype/lions-befrienders-2026-09-11-feedback.md`), one round, 37 Playwright specs (5 new in `tests/e2e/v18.spec.js`), smoke 394 → 415. **M-ADMIN / matching:** manual assign refuses a kaki who does not offer the visit's service with 409 `service_mismatch`; the console shows "⚠ does not offer X" on the roster row and asks a different confirm; a confirmed override is audited as `visit_assigned_service_mismatch` in the same row as the assignment; `best_available` (auto-match, sweep) requires `service_ok`. **M-VISITS:** urgent and soon bookings carry `hours` (1–8, half-hour steps; chips 1, 1.5, 2, 3, 4, 5, 6, 8) and are priced on them. **M-CORE / notify:** the four service notes in `assumptions.json` rewritten for the kaki who receives them, and the family's booking note appended to the assignment message ("The family says: …", trimmed, escaped in email, raw in SMS). **M-USERS:** gender is Female / Male only; one availability entry point ("When I'm free"), dated exceptions headed "Different on a date?" with Day off / Extra day; tier removed from the kaki profile card. **M-CARE:** care plan can be skipped on first setup. **Frontend copy:** sign-in subtitle, role chips ("I'm booking for someone I care for" / "I want to help as a kaki"), "Who are you caring for?", "Kaki arrived", cost stack behind a Show cost toggle, help quick questions (book, the two codes, how long, who to call), all en + zh. **M-HELP:** guide answers for the door codes, time-to-match and who to call, checked before the sign-in fast path; zh keys 门口 / 多久 / 找谁, 取消 ordered before 多久. **Ops, not code:** the SMS sender must be registered (SSIR) before an unaccompanied session; the codes arrived as "Likely-SCAM" on 11 Sept. Review pass caught: the override audit written before the update, `hours` accepted for planned + window, the zh guide matching 多久 in a cancellation question, mid-word note trimming.
- **v1.7 (2026-09-05) — 中文 on the caregiver and kaki screens.** The biggest single ask from the 21 Aug Table Top Exercise. One tap in the brand bar flips every caregiver and kaki screen (sign-in, waiting screen and help panel included) between English and Simplified Chinese; the coordinator console stays English by design. **M-CORE:** `frontend/js/i18n.js` (≈400 ids, `en` + `zh`, plus display names for data values), `UI.t/UI.v/UI.terr/UI.setLang`, `#langBtn`, `errors.KakisError` → `{detail, error}` so senior-facing errors (wrong code, expired code, too many tries, window passed, wrong kaki or start code, not approved) read in the person's language while `detail` stays English; asset URLs `?v=1.7`. **M-USERS:** `users.lang` (migration + checkpoint), `lang` on `PUT /users/me`, `/users/me/profile` and `/auth/me`. **M-AUTH / M-CARE / M-VISITS:** `auth.js`, `caregiver.js`, `kaki.js` carry no literal English; every data value still goes to the API in English; nothing a person typed is translated. Notifications choose en/zh by the recipient's `users.lang` (the emergency contact follows the caregiver); the audit log keeps the English subject. **M-HELP:** `HELP_GUIDE_ZH`, `is_zh`, provider told to answer in the question's language, quick questions and greeting follow the switch. Tests: `tests/e2e/language.spec.js` (8 specs — toggle persists across reload and sign-out/in, admin sees no toggle and an English console under `locale: zh-CN`, a Chinese phone defaults to 中文, caregiver and kaki screens, a full zh lifecycle with a no-English-leak check on every screen and a shared-phone rule, help bot in zh); smoke 368 → 394 (lang round-trip, error codes, zh assignment message with an English caregiver message on the same visit, audit log English, §9.5 holds in messages, zh guide). Two review passes: the first caught a duplicated dictionary key and an admin help panel that could open in Chinese; the second caught names reaching emails unescaped (pre-existing), reasons reaching SMS HTML-escaped, a shared phone able to flip the next person's message language, an English task sentence inside the Chinese assignment message, and guide keywords ordered generic-before-specific. Deployed with backup `kakis.duckdb.bak-20260905-050030`.
